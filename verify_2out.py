    ...
    for j in range(0, j_max):
        for k in range(0, k_max):
            for l in range(0, l_max):
                key = f"i={i}, j={j}, k={k}, l={l}"
                out[key] = {}


                outval = i + j + k + l * data_size
                addr[access] = outval
                vals[access] = key
                out[key][0] = outval
                access += 1
...
print(f"Decoded i={i}, j={j}, k={k}, l={l}")
print(f"Actual {vals[(target_addr - base) // data_size]}")
print(f"Predicted outstmt {outstmt}, value at outstmt {out[key][outstmt]} ---> accessAddr {addr[access_num]}")
